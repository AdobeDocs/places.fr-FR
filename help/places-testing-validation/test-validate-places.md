...
>[!IMPORTANT]
>
>Ce plan suppose que les points d&#39;accès ont été créés dans l&#39;[interface utilisateur du service Places](https://places.adobe.com) et que les dernières versions de l&#39;extension Places et de l&#39;extension du moniteur Places sont installées et correctement configurées. Pour plus d’informations, voir [Place extension](/help/places-ext-aep-sdks/places-extension/places-extension.md) et [Place Monitor extension](/help/places-ext-aep-sdks/places-monitor-extension/places-monitor-extension.md).

| Étape  | Description | Résultat attendu |
|--- |--- |--- |
| 1 | Confirmez que les clés de manifeste appropriées ont été saisies pour Android afin d’accorder l’accès à l’emplacement de suivi. Pour plus d’informations, voir [Ajouter des autorisations pour le manifeste](https://docs.adobe.com/content/help/en/places/using/places-ext-aep-sdks/places-monitor-extension/using-places-monitor-extension.html#add-permissions-to-the-manifest). | Confirmé |
...
| 6 | Créez des fichiers GPX pour chaque point d’intérêt à tester. Les fichiers GPX peuvent être utilisés dans l’environnement de développement local pour simuler une entrée d’emplacement. Pour plus d’informations sur la création et l’utilisation de fichiers GPX, voir ce qui suit : <br>[Fichiers GPX pour le simulateur iOS [fermé]](https://stackoverflow.com/questions/17292783/gpx-files-for-ios-simulator)<br>[https://mapstogpx.com/mobiledev.php](https://mapstogpx.com/mobiledev.php)<br>[TEST D’EMPLACEMENT DANS LES APPLICATIONS MOBILES](https://qacumtester.wordpress.com/2014/02/27/location-testing-in-mobile-apps/) | Les fichiers GPX sont créés et chargés dans le projet d’application. |
...
| 10 | Une fois que vous avez vu le moniteur des emplacements trouver des points d’intérêt proches, vous devez effectuer un test en envoyant des pings d’emplacement. Dans Lancer, créez une règle qui utilise l’extension Lieux pour se déclencher en fonction d’une entrée de géolocalisation. Créez ensuite une nouvelle action en utilisant Mobile Core pour envoyer un postback. La création d’une application Slack Webhook vous permet de voir les entrées et les sorties d’emplacement. Pour plus d&#39;informations sur la création d&#39;une application Webhook Slack, consultez [Envoi de messages à l&#39;aide de hameçons Web entrants.](https://api.slack.com/messaging/webhooks) |  |
| 10 bis | Dans Lancement, veillez à ajouter des éléments de données pour l’extension Places, notamment : <br>Nom actuel de l’API<br>Nom actuel de l’API lat<br>Nom actuel de l’API<br>Nom saisi<br>Dernière entrée lat<br>Dernière entrée longue<br>Dernière sortie<br>Dernière sortie lat<br>Dernière sortie longue<br>Horodatée |  |
| 10 ter | Créer une règle avec un Événement = Lieux → Entrer dans le POI |  |
...
| 12a | Dans les déclencheurs, sélectionnez **Placer le type d&#39;événement - Entrée comme déclencheur**. |  |
| 12 ter | Sélectionnez **[!UICONTROL Placer les métadonnées personnalisées]** comme filtre supplémentaire - utilisez le type d’IPE = Dernière IPE saisie.<br>Nous utilisons  **[!UICONTROL Dernière]** entrée comme type de point d’accès car, dans la plupart des cas,  **[!UICONTROL Dernière]** entrée sera identique à celui de la  **[!UICONTROL valeur actuelle de point d’accès]**. <br><br>**[!UICONTROL Les ]**points d’intérêt actuels ne doivent être utilisés que dans les cas où des clôtures de points d’intérêt se chevauchent. Dans ce cas, ces points d’intérêt doivent être classés, puis le**[!UICONTROL  point d’intérêt actuel ]**affichera le point d’intérêt le plus classé parmi les 2 ou 3 clôtures géographiques dans lesquelles un utilisateur peut se trouver actuellement. |  |
...
| 12e | Pour les clics publicitaires Toujours/Une ou Jusqu’à, sélectionnez *TOUJOURS* afin que vous puissiez effectuer des tests à plusieurs emplacements. | Un message intégré s’affiche TOUJOURS lorsque vous simulez un changement d’emplacement qui répond aux critères de métadonnées appropriés. |
...
| 16a | Dans les déclencheurs, sélectionnez **[!UICONTROL Place le type d&#39;événement]** - **[!UICONTROL Entrée comme déclencheur]**. |  |
| 16 ter | Sélectionnez Places Custom metadata comme filtre supplémentaire - utilisez **[!UICONTROL type POI]** = **[!UICONTROL dernier POI saisi]**. |  |
